...
export const refresh = async (token, { ipAddress, userAgent, correlationId }) => {
  ...
  // ATOMIC UPDATE to mark as used AND revoked (Requirement 5)
...
// ─────────────────────────────────────────────
export const logout = async (sessionId, { correlationId, accessToken, userId } = {}) => {
  if (!sessionId) throw new AppError('Session ID missing', 400, 'LOGOUT_FAILED');
  ...
  await logSecurityEvent({ userId, action: 'LOGOUT', status: 'SUCCESS',
    ip: null, userAgent: null, correlationId, sessionId });
  logger.info('LOGOUT', { userId, jti: sessionId });
};
// ─────────────────────────────────────────────
// GET PROFILE
// ─────────────────────────────────────────────
